...
const getPlatform = () => {
  ...
  const found = Object.keys(platformMap)
    .find(k => appVersion.includes(k));
...
const getEncodingCapabilities = (config, soundCodecs) => {
  ...
  const imageEncodings = config.image_codecs.length > 0
    ? config.image_codecs
    : detectedEncodings;

  const audioEncodings = config.audio_codecs.length > 0
    ? config.audio_codecs
    : soundCodecs;
...
const getClientCapabilities = (config) => {
  ...
  const keycodes = Object.keys(CHARCODE_TO_NAME)
  ...
  return {
    ...
    'screen_sizes': calculateScreens(config.screen[0], config.screen[1], config.dpi),
    'dpi': config.dpi,
