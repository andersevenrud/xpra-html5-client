...
const normalizeWheel = (ev) => {
  ...
  // Legacy
  if ('detail'      in ev) { spinY = ev.detail; }
  if ('wheelDelta'  in ev) { spinY = -ev.wheelDelta / 120; }
  if ('wheelDeltaY' in ev) { spinY = -ev.wheelDeltaY / 120; }
  if ('wheelDeltaX' in ev) { spinX = -ev.wheelDeltaX / 120; }
  ...
  if ('deltaY' in ev) { pixelY = ev.deltaY; }
  if ('deltaX' in ev) { pixelX = ev.deltaX; }
...
export const createMouse = (send, keyboard) => {
  ...
  const process = (ev, surface) => {
    ...
    if (ev.type === 'mousemove') {
      ...
      send('button-action', topwindow, button, pressed, [x, y], modifiers, buttons);
