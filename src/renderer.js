...
const scroll = ({x, y, w, h, data}) => ({drawContext, canvas}, callback) => {
  for (let i = 0, j = data.length; i < j; ++i) {
    const [sx, sy, sw, sh, xdelta, ydelta] = data[i];

    drawContext.drawImage(canvas, sx, sy, sw, sh, sx + xdelta, sy + ydelta, sw, sh);
  }
...
export const createRenderer = ({wid, canvas, context}, send) => {
  ...
  const render = () => {
    if (!listening) {
      ...
      const args = Object.freeze({x, y, w, h, coding, data, sequence, rowstride, options});
